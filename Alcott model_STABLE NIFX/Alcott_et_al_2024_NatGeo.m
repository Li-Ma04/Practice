...
elseif SRP_Pconc > (transition_threshold_P + transition_range_P)
    ...
    PP_P = pars.kPhotoprox * (proportion_P_P * Norm_SRP_P * pars.Redfield_CP + proportion_N_P * norm_total_N_Pconc * pars.Redfield_CN);
end
...
elseif SRP_Dconc > (transition_threshold_D + transition_range_D)
    ...
    PP_D = pars.kPhotodist * (proportion_P_D * Norm_SRP_D * pars.Redfield_CP + proportion_N_D * norm_total_N_Dconc * pars.Redfield_CN);
end
...
elseif SRP_Sconc > (transition_threshold_S + transition_range_S)
    ...
    PP_S = pars.kPhotosurf * (proportion_P_S * Norm_SRP_S * pars.Redfield_CP + proportion_N_S * norm_total_N_Sconc * pars.Redfield_CN);
end
...
NO3_PP_S = 0.3 * N_PP_S * sigmf((log10(NO3_Sconc)),[3,NO3_floor]) ;
NH4_PP_S = 0.7 * N_PP_S * sigmf((log10(NH4_Sconc)),[3,NH4_floor]);
...
% else
%     % In the transition interval, the effects of phosphorus and nitrogen are considered proportionally
%      Pfactor = (SRP_Pconc - (transition_threshold_P - transition_range_P)) / (2 * transition_range_P);
%     
%     % Phosphorus limitation term (when factor = 0)
%     PP_lim_term = 0;
%     
%     % Nitrogen limitation term (when factor = 1)
%     PN_lim_term = NH4_PNfix * ( ( ( SRP_P - (N_P/16)  ) / ( present.SRP_P - (present.N_P/16)    ) )^2 );
%     
%     % Weighted combination of phosphorus and nitrogen effects
%     NH4_Nfix_P = (1 - Pfactor) * PP_lim_term + Pfactor * PN_lim_term;
% end
% 
% 
% 
...
% else
%     % In the transition interval, the effects of phosphorus and nitrogen are considered proportionally
%      Dfactor = (SRP_Dconc - (transition_threshold_D - transition_range_D)) / (2 * transition_range_D);
%     
%     % Phosphorus limitation term (when factor = 0)
%     DP_lim_term = 0;
%     
%     % Nitrogen limitation term (when factor = 1)
%     DN_lim_term = NH4_DNfix * ( ( ( SRP_D - (N_D/16)  ) / ( present.SRP_D - (present.N_D/16)    ) )^2 );
%     
%     % Weighted combination of phosphorus and nitrogen effects
%     NH4_Nfix_D = (1 - Dfactor) * DP_lim_term + Dfactor * DN_lim_term;
% end
% 
% 
% % if (N_D/16) < SRP_D
...
% else
%     % In the transition interval, the effects of phosphorus and nitrogen are considered proportionally
%      Sfactor = (SRP_Sconc - (transition_threshold_S - transition_range_S)) / (2 * transition_range_S);
%     
%     % Phosphorus limitation term (when factor = 0)
%     SP_lim_term = 0;
%     
%     % Nitrogen limitation term (when factor = 1)
%     SN_lim_term = NH4_SNfix * ( ( ( SRP_S - (N_S/16)  ) / ( present.SRP_S - (present.N_S/16)    ) )^2 );
%     
%     % Weighted combination of phosphorus and nitrogen effects
%     NH4_Nfix_S = (1 - Sfactor) * SP_lim_term + Sfactor * SN_lim_term;
% end
% 
% 
% % if (N_S/16) < SRP_S 
